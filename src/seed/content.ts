...
// Full case studies shown in the "Proof of concept" section (mirrors CASES in
// Site.tsx). Seeded into the case-studies collection when it is empty.
export const caseStudiesContent = [
  {
    number: '01',
    order: 1,
    tag: 'Retail · Inventory ops',
    slug: 'retail-inventory-pipeline',
    title: 'Automating custom data pipelines for a retail inventory operation',
    problem:
      'Stock levels were reconciled by hand across a POS system and three supplier feeds. Staff exported spreadsheets nightly, keyed deltas in by morning, and still hit shelf stockouts whenever a supplier changed a SKU mid-week.',
    architecture:
      'A local-first SQLite ingestion layer polls each supplier API on its own schedule. n8n workflows normalize the feeds asynchronously and write reconciled deltas through modular REST endpoints — event-driven, with no nightly batch and no cloud database to rent.',
    outcome:
      'Reconciliation dropped from a multi-hour morning ritual to a continuous background process. Inventory is accurate in near real time, the supplier-SKU drift problem is caught on ingest, and recurring cloud spend is zero.',
    stack: [{ value: 'SQLite' }, { value: 'n8n' }, { value: 'REST endpoints' }, { value: 'Event-driven' }],
    metricBig: 'Hours → minutes',
    metricSub: 'reconciliation cycle',
    published: true,
    featured: false,
  },
  {
    number: '02',
    order: 2,
    tag: 'Legal · Document retrieval',
    slug: 'legal-document-search',
    title: 'Embedded semantic document search for a regional legal practice',
    problem:
      'Thousands of case PDFs lived in nested folders with no real search. Paralegals hunted by filename and memory, and confidentiality rules ruled out shipping client documents to a third-party cloud index.',
    architecture:
      "An embedded vector store runs on the firm's own hardware, with a local LLM generating semantic queries against it. An API-first retrieval service exposes search to the existing case tools — every byte of privileged data stays on premises.",
    outcome:
      'Sub-second semantic retrieval across the full archive, by meaning rather than filename. No documents leave the building, the privacy posture satisfies counsel, and the index scales as new matters are added.',
    stack: [{ value: 'Embedded vectors' }, { value: 'Local LLM' }, { value: 'On-prem' }, { value: 'API-first' }],
    metricBig: '0 documents',
    metricSub: 'leave the premises',
    published: true,
    featured: false,
  },
  {
    number: '03',
    order: 3,
    tag: 'Field service · Orchestration',
    slug: 'field-service-orchestration',
    title: 'Orchestrating quote-to-dispatch for a field service company',
    problem:
      'CRM, scheduling, and invoicing were three disconnected tools. Every job was entered three times, dispatch waited on manual handoffs, and adding any new tool meant another island of double entry.',
    architecture:
      'A modular, API-first backend sits between the existing tools and orchestrates the flow asynchronously: a quote approval fires scheduling, which fires dispatch and invoicing. Endpoints are agent-addressable, so future automation plugs in without a rewrite.',
    outcome:
      'Quote-to-dispatch collapsed to a single triggered flow with no double entry. The company added two new tools later by wiring endpoints — not rebuilding — turning a custom build into a reusable, scalable asset.',
    stack: [{ value: 'Decoupled services' }, { value: 'Async orchestration' }, { value: 'Agent-addressable' }],
    metricBig: '3 entries → 1',
    metricSub: 'quote-to-dispatch',
    published: true,
    featured: false,
  },
]
